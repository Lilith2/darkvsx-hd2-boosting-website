...
export function ServicesProvider({ children }: { children: ReactNode }) {
  ...
  const transformProductToService = (product: any): ServiceData => ({
    ...
    duration:
      product.metadata?.duration || product.estimated_delivery_days
  ...
  const refreshServices = async () => {
    try {
      const serviceProducts = await searchProducts({
        type: "service",
        isActive: true,
      });
      const transformedServices = serviceProducts.map(
        transformProductToService,
      );
  ...
  const addService = async (
    ...
    try {
      await createProduct({
        ...
        base_price: serviceData.original_price
          ? Number(serviceData.original_price)
          : undefined,
        currency: "USD",
        is_active: serviceData.active ?? true,
        features: serviceData.features || [],
        tags: [...(serviceData.popular ? ["popular"] : [])],
        metadata: {
  ...
  const updateService = async (id: string, updates: Partial<Service>) => {
    try {
      ...
      if (updates.original_price)
        updateData.base_price = Number(updates.original_price);
