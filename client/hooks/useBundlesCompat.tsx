...
export function BundlesProvider({ children }: { children: ReactNode }) {
  ...
  const transformProductToBundle = (product: any): BundleData => ({
    ...
    badge: product.tags?.find((tag) =>
      ["featured", "popular", "new"].includes(tag),
    ),
  ...
  const refreshBundles = async () => {
    try {
      const bundleProducts = await searchProducts({
        type: "bundle",
        isActive: true,
      });
  ...
  const updateBundle = async (id: string, updates: Partial<Bundle>) => {
    try {
      ...
      if (updates.discounted_price)
        updateData.price = Number(updates.discounted_price);
      ...
      if (updates.popular !== undefined || updates.badge) {
        ...
        const newTags = [
          ...currentTags.filter(
            (tag) => !["popular", "featured", "new"].includes(tag),
          ),
        ];
