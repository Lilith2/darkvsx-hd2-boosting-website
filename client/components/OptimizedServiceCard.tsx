...
export const OptimizedServiceCard = memo<OptimizedServiceCardProps>(({ 
  ...
  const getDifficultyIcon = useCallback((difficulty: string) => {
    switch (difficulty.toLowerCase()) {
      case 'easy': return <ShieldCheck className="w-4 h-4 text-green-500" />;
      case 'medium': return <Trophy className="w-4 h-4 text-yellow-500" />;
      case 'hard': return <Star className="w-4 h-4 text-red-500" />;
          ...
          <div className="flex-1">
            ...
            <CardDescription className="mt-1 flex items-center gap-2">
              ...
              <div className="flex items-center gap-1">
                ...
                <span className="text-xs text-muted-foreground">{service.difficulty}</span>
              </div>
            </CardDescription>
          </div>
