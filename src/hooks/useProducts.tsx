...
export function useProducts(options: UseProductsOptions = {}) {
  ...
  const fetchProducts = useCallback(async () => {
    try {
      ...
      if (options.popular !== undefined) queryParams.append("popular", options.popular.toString());
  ...
  }, [options.category, options.product_type, options.status, options.featured, options.popular, options.search, options.limit]);
  ...
  const servicesByCategory = useCallback((category?: string) => {
  ...
  const bundles = products.filter(product => product.product_type === 'bundle');
  
  // Legacy compatibility - get services
  const services = products.filter(product => product.product_type === 'service');

  // Legacy compatibility - get custom items
  const customItems = products.filter(product => product.product_type === 'custom_item');
